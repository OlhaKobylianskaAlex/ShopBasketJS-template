...
const createContent = (goods, goodsContainer, cartPrice, cartCounter) => {
  ...
  goodsContainer.parentElement.children[0].innerHTML = `В корзине ${cartCounter} товаров`;
  ...
  if (goods.length > 0) {
    for (let i = 0; i < goods.length; i++) {
      ...
      itemPrice.innerHTML = `цена ${(goods[i].price).toFixed(2)} $`;
      itemSumm.innerHTML = `сумма: ${(goods[i].price * goods[i].count).toFixed(2)} $`;
...
const btnClickHandler = (e) => {
  ...
  if (typeof target !== 'object') {
    console.error('target не объект');
    return;
  };
...
const createBasket = (() => {
  ...
  basket.innerHTML = `
  <div class="goods_header basket">В корзине 0 товаров</div>
  <div class="goods_container basket">
  </div>
  <div class="goods_summ basket">
    <div class="goods_summ-info basket">Всего в корзине товаров на сумму</div>
    <div class="goods_summ-value basket">${cartPrice.toFixed(2)} $</div>
  </div>
  <div class="btn_container basket">
    <button type="button" class="btn btn-primary goods-proceed">Продолжить</button>
    <button type="button" class="btn btn-primary basket goods-delete">Очистить</button>
    <button type="button" class="btn btn-primary basket">Оформить</button>
  </div>`;
